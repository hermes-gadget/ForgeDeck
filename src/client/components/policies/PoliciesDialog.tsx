...
function ValueField({ field, value, bootstrap, onChange }: { field: PolicyField; value: string; bootstrap: Bootstrap; onChange: (value: string) => void }) {
  ...
  if (field === "model") return <label>Value<input list="policy-models" value={value} onChange={(event) => onChange(event.target.value)} /><datalist id="policy-models">{[...bootstrap.models.data, ...(bootstrap.claudeModelOptions || [])].map((model) => <option key={`${model.id}:${model.model}`} value={model.model} />)}</datalist></label>;
