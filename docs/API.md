...
Sessions with `provider: "claude"` run the Claude Code CLI in durable tmux shells. Their models are the CLI aliases `sonnet`, `fable`, `opus`, and `haiku` (locked at creation); `reasoningEffort` is one of `low`, `medium`, `high`, `xhigh`, or `max`; `permissionMode` accepts every Claude Code mode (`default`, `plan`, `acceptEdits`, `bypassPermissions`, `auto`, `manual`, `dontAsk`); and `maxTurns` bounds each turn's agentic loop (1–100, default 15). ForgeDeck maps each thread to a stable Claude session UUID, persists the proven session id inside the tmux session, and resumes the same conversation across server restarts. A read-only workspace lease forces `plan` mode, and `PATCH /policy` toggles between `default` and `bypassPermissions`.

Claude Code stream output includes Anthropic `rate_limit_event` records. ForgeDeck persists those provider observations and gives them precedence in `backendStatus.claude.rateLimit`: a rejected API session limit is 100% until its reported reset time, while an allowed event clears the rejection. Before any provider event is observed, the field falls back to local Claude turn-slot utilization and exposes `source: "local_concurrency"`; provider-derived status exposes `source: "anthropic_api"`. `activeCount` remains a separate count of running ForgeDeck Claude turns and is not treated as the Anthropic API limit.
